...
#include "MarlinConfig.h"
  ...
  void I2CPositionEncoder::update() {
    ...
    if (ec && ecMethod != I2CPE_ECM_NONE) {
        ...
        const uint32_t distance = abs(position - lastPosition),
                       deltaTime = positionTime - lastPositionTime,
      ...
      if (abs(error) > (I2CPE_ERR_CNT_THRESH * planner.axis_steps_per_mm[encoderAxis]) && ELAPSED(millis(), nextErrorCountTime)) {
        ...
        errorCount++;
        nextErrorCountTime = millis() + I2CPE_ERR_CNT_DEBOUNCE_MS;
      }
    }
  ...
  void I2CPositionEncoder::calibrate_steps_mm(const int iter) {
  ...
  int8_t I2CPositionEncodersMgr::I2CPE_idx;
    ...
    if (I2CPE_idx < 0) {
      LOOP_XYZE(i) {
        if (!I2CPE_anyaxis || parser.seen(axis_codes[i])) {
          const int8_t idx = idx_from_axis(AxisEnum(i));
          if (idx >= 0) report_position(idx, hasU, hasO);
        }
      }
    }
    else
  ...
  void I2CPositionEncodersMgr::M863() {
    ...
    const uint8_t iterations = parser.seenval('P') ? constrain(parser.value_byte(), 1, 10) : 1;

    if (I2CPE_idx < 0) {
      LOOP_XYZE(i) {
        if (!I2CPE_anyaxis || parser.seen(axis_codes[i])) {
          const int8_t idx = idx_from_axis(AxisEnum(i));
          if (idx >= 0) calibrate_steps_mm(idx, iterations);
        }
      }
    }
    else
  ...
  void I2CPositionEncodersMgr::M865() {
    ...
    if (!I2CPE_addr) {
      LOOP_XYZE(i) {
        if (!I2CPE_anyaxis || parser.seen(axis_codes[i])) {
          const int8_t idx = idx_from_axis(AxisEnum(i));
          if (idx >= 0) report_module_firmware(encoders[idx].get_address());
        }
      }
    }
    else
    ...
    if (I2CPE_idx < 0) {
      LOOP_XYZE(i) {
        if (!I2CPE_anyaxis || parser.seen(axis_codes[i])) {
          const int8_t idx = idx_from_axis(AxisEnum(i));
          if (idx >= 0)
            enable_ec(idx, onoff == -1 ? !encoders[idx].get_ec_enabled() : !!onoff, AxisEnum(i));
        }
      }
    }
    else
      enable_ec(I2CPE_idx, onoff == -1 ? !encoders[I2CPE_idx].get_ec_enabled() : !!onoff, encoders[I2CPE_idx].get_axis());
  }
